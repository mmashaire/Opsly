...
describe("item updates", () => {
  ...
  it("updates reorderThreshold for an existing item", async () => {
    ...
    const updateResponse = await request(app).patch(`/items/${itemId}`).set(ADMIN_HEADERS).send({
      reorderThreshold: 7,
    });
  ...
  it("returns 404 when updating a non-existent item", async () => {
    const app = createApp();
    const updateResponse = await request(app).patch("/items/missing-item").set(ADMIN_HEADERS).send({
      reorderThreshold: 3,
    });
